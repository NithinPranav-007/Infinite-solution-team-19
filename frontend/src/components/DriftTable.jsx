    ...
    <div className="overflow-hidden rounded-3xl border border-white/10 bg-white/6 shadow-glow backdrop-blur">
      <div className="border-b border-white/10 px-5 py-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <p className="text-sm text-slate-400">Latest scans and detected schema changes</p>
        </div>
      </div>
        ...
        <table className="min-w-full text-left text-sm">
          ...
          <tbody>
            {drifts.length ? drifts.map((drift, index) => (
              <tr key={`${drift.captured_at || index}`} className="border-t border-white/8 text-slate-200 transition hover:bg-white/5">
                ...
                <td className="px-5 py-4 align-top">
                  <span className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs font-medium text-slate-200">
                    {drift.severity || drift.impact_analysis?.severity_label || 'Low'}
                  </span>
                </td>
